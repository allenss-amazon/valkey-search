...
- `INKEYS <count> <key> [<key> ...]` (optional): Restricts results to the specified set of document keys. `<count>` is the number of keys that follow and must be a non-negative integer. Duplicate keys are deduplicated. If `<count>` is 0, no results are returned. The index is not searched: each listed key is evaluated against the query directly, so `total_count` in the response reflects only the listed keys that match the query. For a KNN query, the results are the K nearest of the listed keys that match. Keys that do not exist, or that the index does not cover, are ignored. Every listed key is examined on the main thread, so the cost of a query grows with the number of keys in the list.
